...
const {sendEmailToSpeaker, sendConfirmCancelToAdmin} =
...
module.exports = function(Talk) {
  ...
  Talk.remoteMethod('getTalkDetails', {
    description: 'Gets all talks and returns an object formatted with ' +
    'event info and speaker info.',
    http: {
      path: '/getTalkDetails',
      verb: 'get',
    },
  ...
  Talk.changeTalkContent = function(talkId, newTopic,
    newDescription, newAdminNotes, cb) {
  ...
  Talk.sendEmailToSpeaker = function(adminEmail,
    approved, pending, speakerEmail, speakerName,
    meetupTitle, meetupDate, cb) {
    sendEmailToSpeaker(adminEmail, approved, pending, speakerEmail,
      speakerName, meetupTitle, meetupDate)
  ...
  Talk.afterRemote('talkSubmit', function(ctx, modelInstance, next) {
    ...
    .then((response) => {
      ...
      sendEmailToSpeaker(process.env.MAIN_ADMIN_EMAIL, approved,
  ...
  Talk.afterRemote('changeTalkStatus', function(ctx, modelInstance, next) {
    if (ctx.result.status === 'Approve' || ctx.result.status == 'Deny') {
      ...
      .then((response) => {
        ...
        const meetupDate = response.meetupDate;
        sendEmailToSpeaker(process.env.MAIN_ADMIN_EMAIL, approved,
          pending, speakerEmail, speakerName, meetupTitle,
          meetupDate)
          .then(() => next())
          .catch(err => next(err));
      })
    ...
    ctx.result.status == 'Disengaged') {
      ...
      .then((response) => {
        ...
        const meetupTitle = response.meetupTitle;
        const meetupDate = response.meetupDate;
        sendConfirmCancelToAdmin(confirm, meetupDate,
          meetupTitle, speakerName)
        .then(() => next())
        .catch(err => next(err));
      })
      .catch(err => next(err));
